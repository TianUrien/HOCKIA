-- ============================================================================
-- Phase 3e — MUST-HAVE hard filters for nl-search (discover_profiles)
-- ============================================================================
-- When a recruiter runs Hockia AI search with an active opportunity scope,
-- the opening's MUST-HAVE criteria hard-filter the results (the typed query
-- still wins per-dimension; nl-search only injects a must-have on a dimension
-- the query is silent on). This adds the three must-have filters that weren't
-- cleanly enforceable before:
--
--   p_required_positions          — NULL-neutral position filter. Unlike
--     p_positions (the query path, which drops blank-position candidates as a
--     deliberate "I searched for goalkeepers" intent), this KEEPS a candidate
--     whose position is blank — the honest-absence rule a must-have needs.
--   p_exclude_paid_seekers        — the compensation clash: an unpaid /
--     development must-have excludes candidates who explicitly want paid
--     (mirrors interestFit's only comp hard-fail). NULL/either/development kept.
--   p_required_location_country_id — NULL-neutral location filter: excludes
--     only a candidate who EXPLICITLY excluded the country, or is home-only
--     with a different home (mirrors interestFit's mobility hard-fail). Based-
--     there / open-to-relocate / no-mobility-data are all kept.
--
-- availability (p_available_by) + specialists (p_specialist_skills) are already
-- NULL-neutral hard filters (added in 2e) — nl-search reuses them directly.
-- LEVEL is intentionally NOT enforced here: the verdict's level must-have is
-- PROVEN-band-below-required (proven outranks declared), which needs a
-- world_clubs→world_leagues band join the RPC doesn't carry. Deferred; the
-- recruiter verdict still enforces level.
--
-- Adds 3 trailing params (DEFAULT NULL) → DROP the prior 26-arg overload and
-- recreate (two overloads break PostgREST named-param dispatch).
...
DROP FUNCTION IF EXISTS public.discover_profiles(
  ...
  text, integer[], text, text, date, text[]
);
...
CREATE OR REPLACE FUNCTION public.discover_profiles(
  ...
  -- Phase 3e — active-scope MUST-HAVE hard filters (all NULL-neutral).
  p_required_positions text[] DEFAULT NULL::text[],
  p_exclude_paid_seekers boolean DEFAULT NULL::boolean,
  p_required_location_country_id integer DEFAULT NULL::integer
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
