-- ─────────────────────────────────────────────────────────────────────
-- get_top_community_members — project bio + umpire level/federation + year_founded
-- ─────────────────────────────────────────────────────────────────────
-- QA-fix (Phase 1 carousel unification fallout): the rail renders the unified
-- card + opens the scoped preview, but the RPC never projected the fields those
-- surfaces need:
--   • bio          → the scoped CandidatePreviewSheet showed NO bio for
--                    carousel-sourced candidates (the grid worked via
--                    PROFILES_SELECT).
--   • umpire_level → the umpire card's substance line read "No details yet" in
--                    the Featured-umpires rail (the grid showed "Regional Umpire").
--   • federation   → umpire card location line.
--   • year_founded → club card substance ("Established YYYY").
--
-- Adding columns to RETURNS TABLE changes the signature → DROP + recreate +
-- re-grant. Body is otherwise verbatim from 20260613100100.
...
AS $function$
  SELECT
    ...
    -- QA-fix — fields the unified card / scoped preview need in the rail.
    p.bio,
    p.umpire_level,
    p.federation,
    p.year_founded
