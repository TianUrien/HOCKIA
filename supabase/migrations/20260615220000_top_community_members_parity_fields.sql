-- ─────────────────────────────────────────────────────────────────────
-- get_top_community_members — project the fields the carousel-sourced Preview
-- + scoped recruiter Club Fit need, so a member opened from the Top carousel
-- shows the SAME data (and the SAME match score) as one opened from the All
-- Members grid.
--
-- Audit finding (High): the carousel mapped rows with secondary_position +
-- specialist_skills hardcoded NULL, so computeClubFit ran with no secondary
-- position / specialist skills → the rail's Recruiter Match diverged from the
-- grid's for the same candidate. Also verified_at (verified-badge tooltip) and
-- umpiring_categories (umpire preview) were null. All four are plain profiles
-- columns — project them.
--
-- Adding columns to RETURNS TABLE changes the signature → DROP + recreate +
-- re-grant. Body verbatim from 20260613130000 + the four new projections.
-- ─────────────────────────────────────────────────────────────────────
...
RETURNS TABLE(
  ...
  -- Carousel↔grid parity (drive Club Fit + preview): secondary position +
  -- specialist skills feed computeClubFit; verified_at + umpiring_categories
  -- feed the preview.
