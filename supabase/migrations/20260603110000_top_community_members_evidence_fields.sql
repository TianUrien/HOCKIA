-- ─────────────────────────────────────────────────────────────────────
-- get_top_community_members — also return highlight_video_url +
-- full_game_video_count (Increment #1: Proven lens on the carousel)
-- ─────────────────────────────────────────────────────────────────────
-- The Proven lens (evidence confidence) reads: video (highlight reel /
-- full-match footage), references, verified badge, provable level. The
-- carousel RPC already returns accepted_reference_count, is_verified and
-- current_world_club_id — it's only missing the two VIDEO fields. This
-- adds them so the carousel can render the same ProvenSignal as the grid.
--
-- Pure additive change: identical body / WHERE / ORDER BY to the prior
-- definition (20260603100000); two columns appended to RETURNS TABLE and
