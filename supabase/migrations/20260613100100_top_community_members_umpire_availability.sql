-- ─────────────────────────────────────────────────────────────────────
-- get_top_community_members — also project available_for_appointments
-- ─────────────────────────────────────────────────────────────────────
-- Phase 2 (2b'). The Community carousel renders the unified card, whose umpire
-- "Available" chip now reads `available_for_appointments` (20260613100000)
-- instead of the generic open_to_opportunities. The RPC must project it or the
-- carousel umpire chip stays dark (the All-Members grid selects the column
-- directly, so only the carousel needs this).
--
-- Also fold available_for_appointments into the open-availability OR-clauses so
-- an available umpire counts as "open" for the availability_activity sort + the
-- p_only_open filter (parity with open_to_play/coach/opportunities for the
-- other roles).
--
-- Adding a column to RETURNS TABLE changes the signature → DROP + recreate and
-- re-grant. Body is otherwise verbatim from 20260611100000.
...
AS $function$
  SELECT
    ...
    -- Phase 2 (2b') — umpire appointment availability.
