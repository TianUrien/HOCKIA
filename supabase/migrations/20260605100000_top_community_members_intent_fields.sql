-- ─────────────────────────────────────────────────────────────────────
-- get_top_community_members — return self-declared level_target +
-- opportunity_preference (Matching Increment #4b)
-- ─────────────────────────────────────────────────────────────────────
-- The carousel renders the 🤝 Interested lens, which in #4b scores the
-- candidate's self-declared level + compensation intent against the active
-- opportunity scope. The RPC already returns competition_level_band (the
-- PROVEN level) + the #2.2 mobility fields; add the two self-declared
-- columns so the carousel scores identically to the grid + deep profile.
--
