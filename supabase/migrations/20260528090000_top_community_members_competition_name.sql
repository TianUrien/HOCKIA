-- ─────────────────────────────────────────────────────────────────────
-- get_top_community_members — also return current_competition_name
-- ─────────────────────────────────────────────────────────────────────
-- P1.4: HockeyContextLine renders "{club} · {competition} · {position}"
-- — we already had band (P1.2) and club/position, now we need the
-- league's display NAME too. Server-side join here avoids a per-card
-- client lookup against world_clubs → world_leagues.
--
-- Same join graph as the band derivation: profiles → world_clubs →
-- (men_league_id when player is men/boys, women_league_id when
-- women/girls, fallback to whichever exists). Returns null when the
-- player has no club linked or the club has no league set.
