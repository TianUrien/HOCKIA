-- ─────────────────────────────────────────────────────────────────────
-- Percentages sweep (Admin Portal) — Unit 2 denominators.
--
-- Adds the denominators two activity pages need to show "count + % of
-- denominator", per the founder's request that every metric read with its
-- context:
--   • admin_get_engagement_summary  → total_users (non-test) so the
--     Engagement page can show Active Users (7d/30d) as a share of all users
--     (the activation / monthly-active rate).
--   • admin_get_messaging_health    → summary.active_conversations (non-test
--     conversations with activity in the window) so the Messaging page can
--     show Unanswered as a share of recently-active conversations.
--
-- Both are RETURNS json/jsonb → CREATE OR REPLACE is safe (no signature
-- change); full bodies reproduced so each replace is atomic. The matching
-- numerators already exclude test accounts, so the denominators do too —
-- keeping every ratio ≤ 100%.
-- ─────────────────────────────────────────────────────────────────────

-- ── Engagement summary: + total_users ─────────────────────────────────
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- Denominator for the Active Users tiles (non-test platform users).
    'total_users', (
...
-- ── Messaging health: + summary.active_conversations ──────────────────
CREATE OR REPLACE FUNCTION public.admin_get_messaging_health(
...
BEGIN
  ...
  -- Conversations active in the window (denominator for the Unanswered share).
  -- unanswered ⊆ this (both require last_message_at >= v_since), so the ratio
  -- stays ≤ 100%.
  active_convos AS (
    SELECT COUNT(*) AS active_count
    FROM conversations c
    WHERE c.last_message_at >= v_since
      AND c.participant_one_id NOT IN (SELECT id FROM test_ids)
      AND c.participant_two_id NOT IN (SELECT id FROM test_ids)
  ),
  -- Top messengers
  ...
  SELECT jsonb_build_object(
    'summary', jsonb_build_object(
      ...
      'active_conversations', (SELECT active_count FROM active_convos)
    ),
