...
--
-- Approach: exclude item_type = 'member_joined' from get_home_feed and
-- get_home_feed_new_count RPCs. Existing rows stay in home_feed_items
-- (no data deleted) so the change is fully reversible.
-- ============================================================================
...
DECLARE
  ...
  v_is_test BOOLEAN;
BEGIN
  -- Determine if current user is a test account
  ...
  -- Count total from both sources (with test-account filtering)
  SELECT (
  ...
  FROM (
    ...
    FROM (
      -- System posts (excluding member_joined)
      SELECT
        hfi.created_at,
        hfi.metadata || jsonb_build_object(
          'feed_item_id', hfi.id,
...
COMMENT ON FUNCTION public.get_home_feed IS
  'Returns paginated home feed (system items + user posts). Excludes member_joined cards. Respects test-account visibility.';

-- ── get_home_feed_new_count ─────────────────────────────────────────────────
-- Also exclude member_joined from the "new posts" banner count so it doesn't
-- trigger the banner for join events.
...
BEGIN
  ...
  FROM (
    SELECT created_at FROM home_feed_items
    WHERE deleted_at IS NULL
      AND created_at > p_since
      AND item_type != 'member_joined'
      AND (v_is_test OR is_test_account = false)
    UNION ALL
    SELECT up.created_at FROM user_posts up
    JOIN profiles p ON p.id = up.author_id
    WHERE up.deleted_at IS NULL
...
COMMENT ON FUNCTION public.get_home_feed_new_count IS
  'Counts new feed items since a given timestamp. Excludes member_joined. Respects test-account visibility.';

-- ── Disable the trigger that creates member_joined rows ─────────────────────
-- No point inserting rows that will just be filtered out.
-- Using ALTER TABLE ... DISABLE TRIGGER keeps the function and trigger
-- definition intact so it's easy to re-enable if needed.

ALTER TABLE public.profiles
  DISABLE TRIGGER trigger_member_joined_feed;
