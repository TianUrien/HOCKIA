-- International / representative experience — Step B of the retrieval design
-- (docs/ai/INTERNATIONAL_EXPERIENCE_DESIGN.md §6).
--
-- Adds ONE NULL-neutral parameter to discover_profiles:
--   p_restrict_profile_ids uuid[] DEFAULT NULL
-- When NULL (every existing caller), behaviour is byte-identical. When set,
-- nl-search has pre-resolved verified/self-described profile ids from
-- career_history (entry_type='national_team' / tournaments) and the RPC
-- restricts to them — INSIDE this function, so every existing fence
-- (test-account, blocked, age-gate/frozen-minor, category routing,
-- pagination) still applies. No side path.
--
-- The body below is the LIVE prod definition (md5 b1abe16db6cc2e718a8e1a15
-- 0f866039, verified byte-exact against pg_get_functiondef on 2026-07-27;
-- staging was identical) plus exactly: the appended parameter and the
-- restriction clause in the two WHERE blocks (count + page query).
--
-- Old 29-arg overload is dropped first: CREATE OR REPLACE with a new
-- DEFAULT-ed parameter would otherwise leave two overloads and make every
-- PostgREST rpc() call ambiguous (PGRST203).

DROP FUNCTION IF EXISTS public.discover_profiles(text[], text[], text, integer, integer, integer[], boolean, integer[], text, text, integer, integer, integer[], integer[], text, text, integer, integer, text[], text, text, integer[], text, text, date, text[], text[], boolean, integer);
...
-- Restore the ACL pg_get_functiondef does not carry (verified against
-- prod proacl before the drop: PUBLIC + anon + authenticated + service_role).
GRANT EXECUTE ON FUNCTION public.discover_profiles(text[], text[], text, integer, integer, integer[], boolean, integer[], text, text, integer, integer, integer[], integer[], text, text, integer, integer, text[], text, text, integer[], text, text, date, text[], text[], boolean, integer, uuid[]) TO anon, authenticated, service_role;
