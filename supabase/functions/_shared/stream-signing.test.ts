...
Deno.test('signStreamToken produces an RS256 JWT Cloudflare can verify with the public key', async () => {
  ...
  assertEquals({ sub: payload.sub, kid: payload.kid, exp: payload.exp }, { sub: 'video-uid-123', kid: 'key-abc', exp: 2_000_000_000 })
  assert(typeof payload.nbf === 'number' && payload.nbf <= Math.floor(Date.now() / 1000))
