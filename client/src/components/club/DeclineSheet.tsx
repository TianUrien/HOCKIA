...
export function DeclineSheet({ open, applicationId, firstName, onCancel, onSend }: DeclineSheetProps) {
    ...
    <BottomSheet open={open} onClose={onCancel} ariaLabel={`Decline ${firstName}`}>
      <div className="flex flex-col gap-3.5 px-5 pb-6 pt-1" data-testid="decline-sheet">
        <div className="pt-1">
          ...
          <p className="mt-1 text-[14px] leading-5 text-ink-2">Pick a reason. Hockia writes {firstName} a kind note from it — change anything before it goes.</p>
        </div>
        ...
        <p className="text-caption text-ink-4">This note appears on the application and in the email about it.</p>
        <button
          type="button"
          disabled={!canSend}
