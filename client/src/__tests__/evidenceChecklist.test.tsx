...
describe('evidenceChecklist', () => {
  it('marks present and missing signals for a player', () => {
    ...
    expect(byKey.career.label).toBe('Career History') // recruiter-facing product language
    expect(byKey.open.present).toBe(true)
  })

  it('marks Career History present when the member has career_history entries', () => {
    // Regression: carousel-sourced previews dropped career_entry_count (the
    // RPC omitted it), so a player with real career entries (e.g. 6) showed
    // "Career History: MISSING". The checklist must honour any count > 0.
    const rows = evidenceChecklist({
      role: 'player',
    ...
    expect(career?.label).toBe('Career History')
    expect(career?.present).toBe(true)
  })

  it('keeps the Career History row for coaches with career history', () => {
